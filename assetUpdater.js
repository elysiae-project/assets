...
(async () => {
    ...
    if (apiResponse.status === 200) {
        ...
        for (let i = 0; i < 4; i++) {
            ...
            launcherAssets[gameName] = { videos: [], images: [] };
            let fileIndex = 0;
            await Promise.all(apiData[i].backgrounds.map(async (bgAsset) => {
                const imageURL = bgAsset.background.url;
                const videoURL = bgAsset.video.url;
                for (const assetURL of [imageURL, videoURL]) {
                    ...
                    if (fileExtension === 'webm') {
                        ...
                        await execAsync(`ffmpeg -y -i "${tempFileName}" -vcodec libx264 -tune animation "${outputPath}"`);
                        launcherAssets[gameName].videos.push(outputPath);
                    } else {
                        const outputPath = join("assets", gameName, `${gameName}-${currentIndex}.webp`);
                        await execAsync(`ffmpeg -y -i "${tempFileName}" -lossless 1 -compression_level 6 "${outputPath}"`);
                        launcherAssets[gameName].images.push(outputPath);
                    }
